...
input_font = pygame.font.Font(os.path.join(BASE_DIR, "Fonts/NotoSansJP-Regular.ttf"), 18) # user input
feedback_font = pygame.font.Font(os.path.join(BASE_DIR, "Fonts/NotoSansJP-Regular.ttf"), 16)  # feedback
end_font = pygame.font.Font(os.path.join(BASE_DIR, "Fonts/Bytesized-Regular.ttf"), 80) # game over

# IMAGES
bg_img = pygame.image.load(os.path.join(BASE_DIR, "Images/ppulbatu_background.jpg")).convert() # background image
...
mode = None
buttons = [
    {"label": "Front", "rect": pygame.Rect(SCREEN_WIDTH//2 - 100, SCREEN_HEIGHT//2 - 115, 200, 50)}, # menu buttons
...
while mode is None:
    ...
    for btn in buttons:
        ...
        screen.blit(t, (btn["rect"].centerx - t.get_width() // 2, 
                        btn["rect"].centery - t.get_height() // 2)) # draw buttons
    for event in pygame.event.get():
        if event.type == pygame.QUIT: # allows closing out
            pygame.quit()
        if event.type == pygame.MOUSEBUTTONDOWN:
            for btn in buttons:
                if btn["rect"].collidepoint(event.pos): # button select
                    mode = btn["label"].lower()
    pygame.display.update()

# BUILD CARD DECK BASED ON MODE SELECTED
...
input_box = pygame.Rect(0, SCREEN_HEIGHT - 30, SCREEN_WIDTH, 100) # helps support Japanese IME typing
pygame.key.set_text_input_rect((input_box))  # other option: 0, 740, SCREEN_WIDTH, 40

death_timer = None # initializes timer post death to close game
run = True # game is running
active = False # user is actively clicked into typing
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if input_box.collidepoint(event.pos):
                active = True   # actively typing by click in
            else:
                active = False  # no longer actively typing
        if event.type == pygame.KEYDOWN and state == "playing" and active:
            if event.key == pygame.K_RETURN:
                ...
                if guess and asteroids:
                    ...
                    if guess == target["answer"].strip().lower(): # all lowercase vocab
                        score.points += 10 * level # increasing score by level
                        score.correct += 1
                        asteroids.remove(target)
                        cleared += 1
                        if cleared >= TERMS_PER_LEVEL: # adding levels to increase speed
                            level += 1
                            cleared = 0
                    else:
                        score.incorrect += 1
                        feedback = f"{target['shown']} {target['answer']}" # adding feedback when wrong
                        feedback_timer = 1.0
                        if target["red"]:
                            state = "dead"
                            death_timer = pygame.time.get_ticks()  # start the clock to close game post death
                        else:
                            retry.append({"shown": target["shown"], "answer": target["answer"]}) # retry asteroids
                            asteroids.remove(target)
    ...
    if state == "playing":
        ...
        if spawn_timer >= 2.0:
            spawn_timer = 0
            spawn() # spawn timer for adding new asteroids

        for ast in asteroids[:]:
            ast["y"] += ast["speed"] * delta_time # move asteroid down screen
            if ast["y"] > SCREEN_HEIGHT - 80: # asteroid further than screen goes to retry
                if ast["red"]:
                    state = "dead" # lose game state  (lost retry)
                    death_timer = pygame.time.get_ticks()  # start the clock for quit post death
                else:
                    retry.append({"shown": ast["shown"], "answer": ast["answer"]})
                asteroids.remove(ast)

        if not deck and not retry and not asteroids:
            random.shuffle(cards) # shuffle if all done cards
            deck = cards.copy()
    ...
    for ast in asteroids:
        color = (224, 33, 138) if ast['red'] else (0, 0, 0) # change font if retry asteroid
        img_rect = ast["img"].get_rect(center = (int(ast["x"]), int(ast["y"])))
        screen.blit(ast["img"], img_rect) # add asteroid image
        t = small_font.render(ast["shown"], True, color) # add asteroid text
        screen.blit(t, (int(ast["x"]) - t.get_width() // 2, int(ast["y"]) - t.get_height() // 2)) # center text on asteroid
    ...
    screen.blit(input_font.render(f"Answer: {input_text}|", True, (255, 255, 255)), # for text input bar font
        (20, SCREEN_HEIGHT - 30))
    screen.blit(icon_img, (10, 10)) # add icon image
    score_text = font.render(f"Score: {score.points}", True, (255, 255, 255)) # render score font
    screen.blit(score_text, (SCREEN_WIDTH - score_text.get_width() - 40, # add score text
                        40))
    if feedback:
    ...
    if state == "dead":
        ...
        screen.blit(dead_surf, (SCREEN_WIDTH // 2 - dead_surf.get_width() // 2, SCREEN_HEIGHT // 2)) # add game over once dead

    pygame.display.update() # update display with time
